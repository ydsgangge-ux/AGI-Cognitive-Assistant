...
const Game = {
  ...
  async poll() {
    ...
    try {
      ...
      // Modern mode: show logs; Story mode: logs handled by day_plan panel
      ...
      // Hide non-modern world irrelevant UI elements
      this._applyStoryModeUI(isStoryMode);

      // Main quest progress (non-modern world)
      if (state.life_arc) {
        ...
        if (arc.current_stage) {
          html += `<div style="color:#ccc;margin-bottom:4px;">Current: ${arc.current_stage}</div>`;
        }
        if (arc.stages && arc.stages.length > 0) {
          arc.stages.forEach(s => {
            ...
            html += `<div style="color:${color}">${marker} ${s.name} (${s.duration_days} days)</div>`;
          });
        }
        arcEl.innerHTML = html;
      }

      // Today outline (non-modern world) - progressive, only show reached nodes
      if (state.day_plan && state.day_plan.length > 0 && isStoryMode) {
        ...
        let html = '<div style="color:#a78bfa;margin-bottom:4px;">Today Story <span style="color:#555;font-size:10px;">' + progress + '/' + total + '</span></div>';
        state.day_plan.forEach((item, idx) => {
          // Only show reached nodes, not future ones
