...
- Called by main.py start_server(agent, auth_manager)
- Fully shares the same ConsciousnessAgent instance and memory.db
- Phone login using desktop passphrase, full memory/personality sync
"""
...
try:
    ...
    raise ImportError("Please install deps: pip install fastapi uvicorn")

try:
    import jwt as pyjwt
except ImportError:
    raise ImportError("Please install: pip install PyJWT")

# WeChat Work bridge module (optional, not required)
import wechat_bridge

# -- Global shared instances (injected by start_server) --
_agent        = None
_auth_manager = None
...
_LOGIN_WINDOW = 300     # Time window (seconds)
_LOGIN_LOCKOUT = 60     # Lockout duration (seconds)
...
async def login(req: LoginRequest, response: Response):
    ...
    if len(attempts) >= _LOGIN_MAX:
        ...
        attempts = attempts[-(_LOGIN_MAX - 1):]  # Allow retry
    attempts.append(now)
    ...
    # Login successful, clear records
...
async def chat(req: ChatRequest, current: dict = Depends(_get_current_user)):
    ...
    if _agent is None:
        raise HTTPException(status_code=503, detail="AGI engine not ready")

    user_id = current["user_id"]
    user    = _auth_manager.get_user(user_id) if _auth_manager else None

    # Temporarily switch auth to this user so agent.process() reads correct user_id
...
async def simlife_status(current: dict = Depends(_get_current_user)):
    """Get SimLife status and scene mode"""
...
async def simlife_set_mode(req: SimLifeModeRequest, current: dict = Depends(_get_current_user)):
    """Switch SimLife scene mode (face-to-face)"""
    if not _agent or not _agent.simlife:
        raise HTTPException(status_code=503, detail="SimLife not enabled")
    _agent.simlife_mode = req.enabled
    # Sync to user_profile.json (so SimLife web panel can read it)
    try:
...
# -- External start interface (called by main.py _on_engine_ready) --
def start_server(agent, auth_manager, host="0.0.0.0", port=18765):
    """Start uvicorn in daemon thread, non-blocking for Qt main thread"""
...
<div id="login">
  ...
  <div class="card">
    ...
    <div class="card-sub">Login with desktop passphrase,<br>full memory &amp; personality sync.</div>
    <div class="field">
      <label>Passphrase</label>
      <input id="pp" type="password" placeholder="Enter passphrase..." autocomplete="current-password">
    </div>
...
<div id="chat">
  <div class="topbar">
    <div class="topbar-left">
      ...
      <div class="topbar-info">
        <div class="topbar-name" id="topName">AGI Assistant</div>
        <div class="topbar-status"><div class="dot-live"></div><span id="topUid">Online</span></div>
      </div>
    </div>
    <button class="btn-exit" onclick="doLogout()">Exit</button>
    <button class="btn-scene" id="sceneBtn" onclick="toggleScene()" title="Enter/Leave face-to-face scene">🌿 Scene</button>
  </div>
  ...
  <div class="msgs" id="msgs">
    <div class="empty">
      ...
      <div class="empty-title">Hello</div>
      <div class="empty-sub">How can I help you?</div>
    </div>
  </div>
...
async function doLogin(){
  ...
  if(!pp.trim()){err.textContent='Please enter passphrase';return;}
  ...
  try{
    ...
    if(r.ok){showChat(d.name,'');}else{err.textContent=d.detail||'Incorrect passphrase';}
  }catch(e){err.textContent='Network error, please check connection';}
...
function showChat(name,uid){
  ...
  document.getElementById('topName').textContent=name||'AGI Assistant';
  document.getElementById('topUid').textContent=uid||'Online';
  ...
  try{
    ...
    if(r.ok){
      ...
      if(d.available){
        ...
        if(d.scene_mode){document.getElementById('sceneBtn').classList.add('on');document.getElementById('sceneBtn').textContent='🌿 In Scene';}
        else{document.getElementById('sceneBtn').classList.remove('on');document.getElementById('sceneBtn').textContent='🌿 Scene';}
...
async function toggleScene(){
  ...
  try{
    ...
    if(r.ok){
      if(!isOn){btn.classList.add('on');btn.textContent='🌿 In Scene';}
      else{btn.classList.remove('on');btn.textContent='🌿 Scene';}
    }
  }catch(e){}
}
...
async function send(){
  ...
  }catch(e){t.remove();addRow('agi','⚠️ Network error, please check connection');}
...
function addRow(who,text){
  ...
  el.innerHTML=`<div class="row-label">${who==='me'?'You':'AGI'}  ${now}</div><div class="bubble">${esc(text)}</div>`;
...
function emptyHTML(){return'<div class="empty"><div class="empty-icon"><svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/></svg></div><div class="empty-title">Hello</div><div class="empty-sub">How can I help you?</div></div>';}
